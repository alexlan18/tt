    ...
    $.addFlex = function (t, p) {
        ...
        p = $.extend({ //apply default properties
            ...
            __mw: { //extendable middleware function holding object
                datacol: function(p, col, val, row) { //middleware for formatting data columns
                    var _col = (typeof p.datacol[col] == 'function') ? p.datacol[col](val, row) : val; //format column using function
            ...
            datacol: {}, //datacol middleware object 'colkey': function(colval, id) {}
        ...
        var g = {
            ...
            fixHeight: function (newH) {
                ...
                if (nd > newH && newH != 0) $(g.nDiv).width(200);
            ...
            addData: function (data) { //parse data
                ...
                if (p.dataType == 'json') {
                    $.each(data.rows, function (i, row) {
                        ...
                        if (row[p.idProperty]) {
                            tr.id = p.id + 'row' + row[p.idProperty];
                        }
                        $('thead tr:first th', g.hDiv).each( //add cell
                            function () {
                                ...
                                if (typeof row.cell == 'undefined') {
                                    ...
                                    td.innerHTML = p.__mw.datacol(p, $(this).attr('abbrName'), iHTML, tr); //use middleware datacol to format cols
                                }
                    ...
                    $("rows row", data).each(function () {
                        ...
                        $('thead tr:first th', g.hDiv).each(function () {
                            ...
                            td.innerHTML = p.__mw.datacol(p, $(this).attr('abbr'), text, tr); //use middleware datacol to format cols
                            $(td).attr('abbr', $(this).attr('abbr'));
                            $(tr).append(td);
                            td = null;
                        });
        ...
        g.mask = document.createElement('div'); //create window-mask
        ...
        //新增遮罩效果
        g.mask.className = "flexigrid-mask";
        $(g.gDiv).before(g.mask);
        $(g.mask).hide();
        ...
        $('thead tr:first th', g.hDiv).each(function () {
            ...
            if ($(this).attr('abbr')) {
                ...
                if($(this).find(":checkbox").length > 0) {
                    ...
                    //by panwb 绑定click事件，全选复选框
                    $(this).click(function(event) {
                        if($(this).find(":checkbox").prop("checked")) {
                            $(this).find(":checkbox").prop("checked", false);
                            $(g.bDiv).find(":checkbox").prop("checked", false);
                        } else {
                            $(this).find(":checkbox").prop("checked", true);
                            $(g.bDiv).find(":checkbox").prop("checked", true);
                        }
                    });
                }
            }
            ...
            $(this).empty().append(thdiv).removeAttr('width').mousedown(function (e) {
                ...
                if($(this).find(":checkbox").length == 0) {
                    ...
                    if (g.colCopy) {
                        ...
                        //modify by panwb (g.gDiv.offsetTop) (g.gDiv.offsetParent.offsetLeft)

                        var ndw = parseInt($(g.nDiv).width(), 10);
                        $(g.nDiv).css({
                            top: g.gDiv.offsetTop + g.bDiv.offsetTop  - document.body.scrollTop
                        });
                        if ((nl + ndw) > $(g.gDiv).width()) {
                            $(g.nDiv).css('left', onl - ndw + 1 + g.gDiv.offsetParent.offsetLeft);
                        } else {
                            $(g.nDiv).css('left', nl + g.gDiv.offsetParent.offsetLeft);
                        }
        ...
        //by panwb --- 为checkbox添加点击事件
        $('thead tr:first th :checkbox', g.hDiv).each(function () {
