(function($) {
	...
	function reSize(jq) {
		...
		t._outerWidth(options.width);
		t._outerHeight(options.height);
	...
	function wrapCalendar(jq) {
		...
		$(jq).addClass("calendar").wrapInner(
				"<div class=\"calendar-header\">" + "<div class=\"calendar-prevmonth\"></div>" + "<div class=\"calendar-nextmonth\"></div>" + "<div class=\"calendar-prevyear\"></div>" + "<div class=\"calendar-nextyear\"></div>"
						+ "<div class=\"calendar-title\">" + "<span>Aprial 2010</span>" + "</div>" + "</div>" + "<div class=\"calendar-body\">" + "<div class=\"calendar-menu\">" + "<div class=\"calendar-menu-year-inner\">"
						+ "<span class=\"calendar-menu-prev\"></span>" + "<span><input class=\"calendar-menu-year\" type=\"text\"></input></span>" + "<span class=\"calendar-menu-next\"></span>" + "</div>"
	...
	function showMenu(jq) {
		...
		if ($(jq).find(".calendar-menu-month-inner").is(":empty")) {
			...
			for ( var i = 0; i < 3; i++) {
				...
				for ( var j = 0; j < 4; j++) {
					$("<td class=\"calendar-menu-month\"></td>").html(options.months[idx++]).attr("abbr", idx).appendTo(tr);
				}
			}
			// 菜单内的上年、下年按钮hover事件
		...
		monthInner.find("td:eq(" + (options.month - 1) + ")").addClass("calendar-selected");
		// 设置菜单大小
		// 当前页面中浏览器是否使用标准盒模型渲染页面,盒子模型是CSS的一个布局概念，详情参考http://www.w3school.com.cn/css/css_boxmodel.asp
		menu._outerWidth(body.outerWidth());
		menu._outerHeight(body.outerHeight());
		menu._outerHeight(menu.height() - yearInner.outerHeight());
	}
	;
	/**
	 * 根据年月获取日历所要显示的日期数组
	...
	function renderCalendar(jq) {
		...
		// 创建“周”头部
		for ( var i = options.firstDay; i < options.weeks.length; i++) {
			tr.append("<th>" + options.weeks[i] + "</th>");
		}
		// 循环日期数组，逐行逐列渲染日期
		for ( var i = 0; i < options.firstDay; i++) {
			tr.append("<th>" + options.weeks[i] + "</th>");
		}
		var dateArray = getDateArray(jq,options.year, options.month);// 根据年月获取需要显示的日期数组
		for ( var i = 0; i < dateArray.length; i++) {
			var week = dateArray[i];
			var tr = $("<tr></tr>").appendTo(t.find("tbody"));
			for ( var j = 0; j < week.length; j++) {
				var date = week[j];
				 $("<td class=\"calendar-day calendar-other-month\"></td>").attr("abbr", date[0] + "," + date[1] + "," + date[2]).html(date[2]).appendTo(tr);
			}
		}
		// 当月日期和非当月日期样式设置
		t.find("td[abbr^=\"" + options.year + "," + options.month + "\"]").removeClass("calendar-other-month");
		var now = new Date();
		var dateStr = now.getFullYear() + "," + (now.getMonth() + 1) + "," + now.getDate();
		t.find("td[abbr=\"" + dateStr + "\"]").addClass("calendar-today");
		if (options.current) {
			t.find(".calendar-selected").removeClass("calendar-selected");
			var currentDateStr = options.current.getFullYear() + "," + (options.current.getMonth() + 1) + "," + options.current.getDate();
			t.find("td[abbr=\"" + currentDateStr + "\"]").addClass("calendar-selected");
		}
		var first = 6 - options.firstDay;
		var last = first + 1;
		if (first >= 7) {
			first -= 7;
		}
		...
		t.find("tr").find("td:eq(" + first + ")").addClass("calendar-saturday");
		t.find("tr").find("td:eq(" + last + ")").addClass("calendar-sunday");
		// 日期单元格hover、click事件
