(function($) {
	 ...
	 *            jq
	 * @param {Object}
	 *            tag 是否校验（true:是,flase:否）
	 */
	function validateValue(jq, tag) {
		var options = $.data(jq, "combo").options;
		var textBox = $(jq).combo("textbox");
		textBox.validatebox($.extend({},options,{deltaX:(options.hasDownArrow?options.deltaX:(options.deltaX>0?1:-1)), combo : jq}));
		if (tag) {
	...
	$.fn.combo.parseOptions = function(target) {
        ...
        //panwb---增加对data-options的支持
        var options = $.parser.parseOptions(target, [ "id", "width", "height", "left", "top", "title", "iconCls", "cls", "headerCls", "bodyCls", "tools", "href"]);
		return $.extend(options, $.fn.validatebox.parseOptions(target), {
			...
			editable : (t.attr("isReadOnly")=="true" ? false : (t.attr("editable") ? t.attr("editable") == "true" : undefined)),
			isReadOnly : (t.attr("isReadOnly") ? t.attr("isReadOnly") == "true" : undefined),
