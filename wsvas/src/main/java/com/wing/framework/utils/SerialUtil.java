...
/**
   ...
   * 生成的格式是: 200908010001 前面几位为当前的日期,后面四位为系统自增长类型的编号
   * 原理:
   *      1.获取当前日期格式化值;
...
public class SerialUtil {
	//获取自增长ID   规则：1位的typeID + 8位的日期  + 4位的自增数
	public static String getSerialId(String typeID){
        SerialNumber serial = new FileEveryDaySerialNumber(5, "EveryDaySerialNumber");
        return typeID+serial.getSerialNumber();
	}
}
