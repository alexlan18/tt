(function($) {
	 ...
	 *            values
	 * @param {Object}
	 *            single 是否单选（这个参数感觉没完全吃透）
	 */
	function setValues(jq, values, single) {
		var options = $.data(jq, "combobox").options;
		var data = $.data(jq, "combobox").data;// 获取combobox的数据
		var panel = $(jq).combo("panel");
		panel.find("div.combobox-item-selected").removeClass("combobox-item-selected");// 去掉当前值选项的选中样式
		var vv = [], ss = [];
		for ( var i = 0; i < values.length; i++) {
			var v = values[i];
			var s = v;
	...
	function reloadData(jq, url, paramData, single) {
		...
		if (options.dropdown) {
			loadData(jq, $(options.dropdown).dropdown("getData"), single);
			return;
		}
		if (!options.url) {
			return;
	...
	$.fn.combobox.methods = {
		...
		clear : function(jq) {
			return jq.each(function() {
                ...
                panel.find("div.combobox-item-selected").removeClass("combobox-item-selected");// 去掉选中样式
	...
	$.fn.combobox.parseOptions = function(target) {
		...
		return $.extend({}, $.fn.combo.parseOptions(target), {
			valueField : t.attr("valueField") || (t.attr("dropdown") ? $(t.attr("dropdown")).dropdown("options").valueField : undefined),
			textField : t.attr("textField") || (t.attr("dropdown") ? $(t.attr("dropdown")).dropdown("options").textField : undefined),
			mode : t.attr("mode"),
			method : (t.attr("method") ? t.attr("method") : undefined),
			url : t.attr("url"),
			dropdown : t.attr("dropdown")
		});
	};
	/**
	 * 默认参数设置
