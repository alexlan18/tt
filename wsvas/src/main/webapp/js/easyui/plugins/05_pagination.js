	...
	function buildToolbar(target) {
		...
		var pager = {
			...
			prev : {
				...
				handler : function() {
					if (opts.pageNumber > 1) {
						setSelect(target, opts.pageNumber - 1);
					}
				}
			},
			next : {
				...
				handler : function() {
					var totolNumber = Math.ceil(opts.total / opts.pageSize);
					if (opts.pageNumber < totolNumber) {
						setSelect(target, opts.pageNumber + 1);
					}
				}
			},
		...
		function setPagination(indexPage) {
			...
			a.wrap("<td></td>");
			a.linkbutton({
				iconCls : pager[indexPage].iconCls,
				plain : true
			}).unbind(".pagination").bind("click.pagination", pager[indexPage].handler);
			return a;
		}
		;
